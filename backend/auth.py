...
def validate_nurse_credentials(email: str, password: str, db) -> dict:
    ...
    # Query nurses collection directly
    ...
    if not stored_password or not isinstance(stored_password, str):
        return None  # Account has no valid password hash

    # Guard against non-bcrypt hashes (e.g. plain text or werkzeug hashes)
    if not stored_password.startswith('$2b$') and not stored_password.startswith('$2a$'):
        return None  # Invalid hash format — account needs re-registration

    try:
        if User.check_password(stored_password, password):
            return nurse
    except Exception:
        pass  # Never leak bcrypt errors (e.g. 'Invalid salt') to the client
    return None
